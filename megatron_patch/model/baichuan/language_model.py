...
class TransformerLanguageModel(MegatronModule):
    ...
    def __init__(self,
        ...
        self.max_cache_pos = args.max_padding_length
        # Embeddings.
        if self.pre_process:
    ...
    def forward(self, enc_input_ids, enc_position_ids, enc_attn_mask,
        ...
        if inference_params is not None:
            if self.alibi_mask is None or self.alibi_mask.shape[-1] != self.seq_length_with_past:
                self.alibi_mask = self.get_alibi_mask(encoder_input, self.seq_length_with_past)
            alibi_mask = self.alibi_mask
        else:
            alibi_mask = self.get_alibi_mask(enc_input_ids, self.seq_length_with_past)

        if not self.use_alibi_mask:
            ...
            bsz = enc_input_ids.size(0)
            src_len, tgt_len = alibi_mask.size()[-2:]
            expanded_mask = expanded_mask.expand(bsz, 1, src_len, tgt_len).to(alibi_mask.dtype)
            inverted_mask = 1.0 - expanded_mask
            inverted_mask = inverted_mask.masked_fill(inverted_mask.to(torch.bool), torch.iinfo(alibi_mask.dtype).min)

...
from megatron.async_checkpoint import commit_checkpoint
...
def get_nondistributed_optimizer_checkpoint_name(model_checkpoint_name):
    return os.path.join(os.path.dirname(model_checkpoint_name),
                        "nondistrib_optim.pt")
...
def get_rng_state():
    ...
    if dist.is_initialized() and \
        ...
        if not args.optim_no_allgather:
...
def save_optimizer_state(optimizer, opt_param_scheduler, checkpoint_name, args=None):
    """Force save opimizer states, ignoring no_save_otimizer"""
    
    if args is None:
        args = get_args()

    if args.use_distributed_optimizer and optimizer is not None:
    ...
    if not torch.distributed.is_initialized() \
       ...
       or args.expert_parallel:
        optim_checkpoint_name = \
            get_nondistributed_optimizer_checkpoint_name(checkpoint_name)
        state_dict = {}
        ...
        ensure_directory_exists(optim_checkpoint_name)
        commit_checkpoint(state_dict, optim_checkpoint_name)

def save_model_weights(iteration, model, checkpoint_name, args=None):
    """Force save model weights, ignoring no_save_weights"""
    ...
    if not torch.distributed.is_initialized() \
       ...
       or args.expert_parallel:
        ...
        if not args.no_save_rng:
            state_dict["rng_state"] = get_rng_state()

        # Save.
        ensure_directory_exists(checkpoint_name)
        commit_checkpoint(state_dict, checkpoint_name)
...
def _load_optim_checkpoint(load_dir, rank0=False):
    tracker_filename = get_checkpoint_tracker_filename(load_dir)
    ...
    assert not release
    if rank0:
        weight_checkpoint_name = find_checkpoint_rank_0(load_dir, iteration, release)
    else:
        weight_checkpoint_name = get_checkpoint_name(load_dir, iteration, release)
    checkpoint_name = get_nondistributed_optimizer_checkpoint_name(weight_checkpoint_name)
...
def load_checkpoint(model, optimizer, opt_param_scheduler, load_arg='load', strict=True):
    ...
    if not release and not args.finetune and not args.no_load_optim:
        try:
            # Load state dict.
            optim_dict, checkpoint_name = _load_optim_checkpoint(load_dir)
            if optimizer is not None:
            ...
            # Load scheduler.
            if opt_param_scheduler is not None:
                if 'lr_scheduler' in optim_dict: # backward compatbility
                    opt_param_scheduler.load_state_dict(optim_dict['lr_scheduler'])
                else:
                    opt_param_scheduler.load_state_dict(optim_dict['opt_param_scheduler'])
        except KeyError:

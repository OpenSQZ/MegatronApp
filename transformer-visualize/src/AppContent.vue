...
import { FlashOutline, CodeDownloadOutline, AddCircleOutline, RemoveCircleOutline, LayersOutline, InformationCircleOutline } from "@vicons/ionicons5"; // Added more icons
...
// [layer_idx][batch_idx][query_token_idx][key_token_idx]
...
const { status: wsStatus, data: wsData, send: wsSend, open: wsOpen, close: wsClose } = useWebSocket(
  "ws://localhost:8765", // 后端 WebSocket 地址
  {
    ...
    onMessage: (ws, event) => {
      ...
      if (type === "start") {
        ...
        // --- Attention Matrix (Type 2) ---
          ...
          const n_dim_queries = parseInt(parsedData.args[0]); // num new queries for this update
          const m_dim_keys = parseInt(parsedData.args[1]);    // total key sequence length now
          const flat_data: number[] = parsedData.result;
          ...
          for (let bIdx = 0; bIdx < currentBatchSize; bIdx++) {
            if (!attention_matrix_batched.value[layer_idx][bIdx] || attention_size_batched.value[layer_idx][bIdx] === undefined) {
                 ...
                 attention_matrix_batched.value[layer_idx][bIdx] = []; // Initialize if needed
            ...
            if (m_dim_keys > current_matrix_size) {
              ...
              for (let r = 0; r < current_matrix_size; r++) {
                batch_matrix[r].push(...Array(m_dim_keys - current_matrix_size).fill(0));
              }
              // Add new rows
              for (let r = 0; r < (m_dim_keys - current_matrix_size); r++) {
                batch_matrix.push(Array(m_dim_keys).fill(0));
              }
            ...
            const data_offset_for_batch = bIdx * n_dim_queries * m_dim_keys;

            for (let i = 0; i < n_dim_queries; i++) { // Iterate over new query rows
              const matrix_row_idx = start_row_idx + i;
              if (matrix_row_idx < batch_matrix.length) {
                for (let j = 0; j < m_dim_keys; j++) { // Iterate over key columns
                  if (j < batch_matrix[matrix_row_idx].length) {
                    const data_idx = data_offset_for_batch + i * m_dim_keys + j;
                    if (data_idx < flat_data.length) {
                      batch_matrix[matrix_row_idx][j] = flat_data[data_idx];
                    } else { console.warn(`Attn Update: Flat data OOB. Idx:${data_idx}, Len:${flat_data.length}`); }
                  } else { console.warn(`Attn Update: Key idx ${j} OOB for row ${matrix_row_idx}. Row len: ${batch_matrix[matrix_row_idx].length}`);}
                }
              } else { console.warn(`Attn Update: Query idx ${matrix_row_idx} OOB. Matrix rows: ${batch_matrix.length}`); }
            }
          }
        }
...
const tokens_for_selected_batch_attention_and_pca = computed(() => {
...
const getAttentionMatrixForDisplay = (layerId: number) => {
  if (!isInitializedAfterStart.value) return [];
  const layerIdx = layerId - 1;
  const bIdx = current_batch_index_to_display.value;
  if (layerIdx < 0 || layerIdx >= actualNumLayers.value ||
      bIdx < 0 || bIdx >= batch_size.value ||
      !attention_matrix_batched.value[layerIdx] || !attention_matrix_batched.value[layerIdx][bIdx]) {
    return [];
  }
  return attention_matrix_batched.value[layerIdx][bIdx];
};
        ...
        <n-tab-pane name="attention" tab="注意力矩阵"  v-if="visualizationSwitches.RawAttentionScore_mat_mul">
            ...
            <n-gi v-for="layerId in attentionLayersToDisplay" :key="`attn-${layerId}`" span="2 m:1">
                ...
                v-if="getAttentionSizeForDisplay(layerId) > 0 && 
                        tokens_for_selected_batch_attention_and_pca.length >= getAttentionSizeForDisplay(layerId) &&
                        getAttentionMatrixForDisplay(layerId).length === getAttentionSizeForDisplay(layerId)"
                :values="getAttentionMatrixForDisplay(layerId)"
                :size="getAttentionSizeForDisplay(layerId)"
                :color="attention_color"
                :layer_id="layerId"
                :tokens="tokens_for_selected_batch_attention_and_pca.slice(0, getAttentionSizeForDisplay(layerId))"
                />
                <n-empty v-else :description="`层 ${layerId} Batch ${current_batch_index_to_display + 1} 注意力数据不可用或维度不匹配 (矩阵大小: ${getAttentionSizeForDisplay(layerId)}, Tokens: ${tokens_for_selected_batch_attention_and_pca.length}, 实际矩阵行数: ${getAttentionMatrixForDisplay(layerId).length})`" style="padding: 20px 0;" />
            </n-gi>
            </n-grid>

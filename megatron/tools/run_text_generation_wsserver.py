...
from megatron.text_generation_wsserver import MegatronWSServer
...
if __name__ == "__main__":
    ...
    if mpu.is_pipeline_first_stage() and mpu.get_tensor_model_parallel_rank() == 0:
        server = MegatronWSServer(model)
        server.run("0.0.0.0",port=args.port)

    while True:
        from megatron import get_flags, get_disturbance
        obj_list = [None]
        torch.distributed.broadcast_object_list(obj_list, 0)
        get_flags().set_by_configs(obj_list[0])
        torch.distributed.broadcast_object_list(obj_list, 0)

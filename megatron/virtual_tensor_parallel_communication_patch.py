...
# Default Ops:
all_gather_object = dist.all_gather_object
...
def all_gather_object(object_list, obj, group=None):
    ...
    if not use_thread_communication:
        if group is None:
            ...
            with global_condition:
                global_request_group = group
                global_request_args = (object_list, obj)
                global_request_func = dist.all_gather_object
                while global_request_func is not None:
                    global_condition.wait()
        elif len(group) == 3:
            ...
            # print(group)
            with request_conditions[0]:
                requests_queue.put((0, dist.all_gather_object, (object_list, obj), group))
                while controller.get_status(0):
                    request_conditions[0].wait()
    elif isinstance(group, int):
        lock.acquire()
        ...
        finished_thread[index] = 1
        with request_conditions[index]:
            requests_queue.put((index, dist.all_gather_object, (object_list, obj), group))
            while controller.get_status(index):
                request_conditions[index].wait()
...
def broadcast(tensor, src, group=None):
    ...
    if not use_thread_communication:
        ...
        if group is None:
            ...
            with global_condition:
                global_request_group = group
                global_request_args = (tensor, src)
        ...
        if get_thread_index() == target:
            with global_condition:
                global_request_group = group
                global_request_args = (tensor, get_real_rank(src))
                global_request_func = dist.broadcast
                while global_request_func is not None:
                    global_condition.wait()
            result = tensor

...
from megatron import get_tracers, get_flags, FlagType, get_disturbance
...
class ParallelMLP(MegatronModule):
    ...
    def __init__(self, config, is_expert=False, layer_number=1):
        ...
        self.dense_h_to_4h = tensor_parallel.ColumnParallelLinear(
            ...
            skip_bias_add=True,
            is_expert=is_expert,
            layer_flag_type=(layer_number, FlagType.MLP1_mat_mul),
        )
        ...
        self.dense_4h_to_h = tensor_parallel.RowParallelLinear(
            ...
            is_expert=is_expert,
            layer_flag_type=(layer_number, FlagType.MLP2_mat_mul),
        )
...
class CoreAttention(MegatronModule):
    ...
    def forward(self, query_layer, key_layer,
        ...
        if get_flags().get_flag(FlagType.RawAttentionScore_mat_mul, self.layer_number):
            tracers.tik_tensor((self.layer_number, FlagType.RawAttentionScore_mat_mul), attention_probs)
        ...
        if get_flags().get_flag(FlagType.ContextLayer_mat_mul, self.layer_number):
            tracers.tik_tensor((self.layer_number, FlagType.ContextLayer_mat_mul), context_layer.transpose(0, 1))
    ...
    def __init__(self, config, layer_number,
        ...
        if attention_type == AttnType.self_attn:
            self.query_key_value = tensor_parallel.ColumnParallelLinear(
                ...
                gather_output=False,
                layer_flag_type=(self.layer_number, FlagType.QKV_mat_mul),
                )
        else:
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def forward(self, hidden_states, attention_mask,
        ...
        if get_disturbance().calculation_perturbation and get_disturbance().calculation_perturbation_fn is not None:
            mlp_output = get_disturbance().calculation_perturbation_fn(mlp_output)

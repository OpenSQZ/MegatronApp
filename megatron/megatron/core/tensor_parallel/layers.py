...
class ColumnParallelLinear(torch.nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        layer_flag_type = (0, FlagType.INVALID_FLAG),
    ):
        super(ColumnParallelLinear, self).__init__()

        self.layer_flag_type = layer_flag_type
    ...
    def forward(self, input_: torch.Tensor, weight: Optional[torch.Tensor] = None):
        ...
        if get_flags().get_flag(self.layer_flag_type[1], self.layer_flag_type[0]):
            if output_bias is not None:
                get_tracers().tik_tensor(self.layer_flag_type, (output + output_bias).transpose(0, 1))
            else:
                get_tracers().tik_tensor(self.layer_flag_type, output.transpose(0, 1))

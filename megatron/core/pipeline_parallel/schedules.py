...
def get_forward_backward_func(use_dpp, multi_node):
...
def backward_step(input_tensor, output_tensor, output_tensor_grad, model_type, config, virtual_microbatch):
    ...
    *,
    ...
    with no_sync_func():
        for i in range(num_microbatches - 1):
            ...
            if not forward_only:
                backward_step(input_tensor, output_tensor, output_tensor_grad, model_type, config, i)
    ...
    if not forward_only:
        backward_step(input_tensor, output_tensor, output_tensor_grad, model_type, config, num_microbatches - 1)
    ...
    def backward_step_helper(virtual_microbatch_id):
        ...
        with tracers.scope("backward", microbatch=microbatch_id_table[virtual_microbatch_id], model_chunk=model_chunk_id, pp_rank=pipeline_parallel_rank):
            input_tensor_grad = backward_step(
                input_tensor, output_tensor, output_tensor_grad, model_type, config, virtual_microbatch_id
            )
    ...
    def forward_step_helper(
        ...
        with tracers.scope("forward", microbatch=microbatch_id_table[virtual_microbatch_id], model_chunk=model_chunk_id, pp_rank=pipeline_parallel_rank):
            output_tensor, num_tokens = forward_step(
                ...
                current_microbatch=microbatch_id,
                virtual_microbatch=virtual_microbatch_id,
            )
    ...
    def backward_step_helper(virtual_microbatch_id, dual_index):
        ...
        with tracers.scope("backward", microbatch=microbatch_id_table[virtual_microbatch_id], model_chunk=model_chunk_id, pp_rank=pipeline_parallel_rank):
            output_tensor = backward_step(
                forward_ready_tensors[dual_index], forward_finished_tensors[dual_index], backward_ready_tensors[virtual_microbatch_id], model_type, config, virtual_microbatch_id
            )

...
class TransformerBlock(MegatronModule):
    ...
    def _forward_and_backward_func(
        self,
        attention_mask: Tensor,
        context: Tensor,
        context_mask: Tensor,
        ...
        if self.config.recompute_method == 'uniform':
            ...
            # the input activation of each divided chunk.
            # A method to further reduce memory usage reducing checkpoints.
            layer_idx = self.num_layers_per_pipeline_rank - self.config.recompute_num_layers
            idx = 0
            while layer_idx >= 0:
                output, context = checkpoint_handler(
                    custom(layer_idx, layer_idx + self.config.recompute_num_layers), hidden_states[idx]
                )

                output.backward(gradient = output_grad)

                output_grad = hidden_states[idx].grad
    ...
    def forward_and_backward(
        ...
        *,
        ...
        with rng_context, outer_fp8_context:
            # Forward pass.
            if self.config.recompute_granularity == 'full' and self.training:
                hidden_states = self._forward_and_backward_func(
                    hidden_states=hidden_states,
                    attention_mask=attention_mask,
                    context=context,
                    context_mask=context_mask,

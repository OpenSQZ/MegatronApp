...
aggregated_average_sending_windows = []
for pp_type in ["pp", "dpp"]:
    ...
    for forward_send_rank in range(pp_ranks):
        ...
        avg_sending_windows.append(sum(sending_windows) / len(sending_windows) / 1000)
    for backward_send_rank in range(pp_ranks):
        ...
        avg_sending_windows.append((sum(sending_windows) / len(sending_windows)) / 1000)
    aggregated_average_sending_windows.append(avg_sending_windows)
pairs = ["0-1", "1-2", "2-3", "3-0", "0-3", "1-0", "2-1", "3-2"]
pp_data = aggregated_average_sending_windows[0]
dpp_data = aggregated_average_sending_windows[1]
...
plt.ylabel("Send Window (ms)")
plt.title(args.trace_dir.split("/")[-1] + " pp vs dpp")
plt.xticks(x, pairs)
plt.legend()
plt.tight_layout()
plt.savefig(f"plots/{args.trace_dir.split('/')[-1]}-sending-windows-aggregated.png")
...
- Other critical parameters include the number of layers of the model (note that currently the value is 16 and is static in `scripts/launchers/run_single_<model>.sh`, so update the launcher script if adjusting the layers), the global batch size and the sequence length

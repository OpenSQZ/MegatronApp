...
from megatron.trace import tracers as tra
...
def forward_backward_pipelining_with_interleaving(
    *,
    ...
    forward_ready_tensors = manager.list([None] * total_num_microbatches)
    forward_finished_tensors = manager.list([None] * total_num_microbatches)
    backward_ready_tensors = manager.list([None] * total_num_microbatches)
    backward_finished_tensors = manager.list([None] * total_num_microbatches)"""
    ...
    """def forward_send(forward_finished_tensors):
        forward_sent_indices = []
        # torch.cuda.set_device(torch.distributed.get_rank())
        forward_sent_count = 0
        while True:
            has_tensor_to_send = False
            for i in range(num_model_chunks): # chunk number in one GPU
                for j in range(i * pipeline_parallel_size, total_num_microbatches, pipeline_parallel_size * num_model_chunks):
                    for k in range(j, j + pipeline_parallel_size):
                        if (
                            forward_finished_tensors[k] is not None
                            and k not in forward_sent_indices
                            and not (
                                parallel_state.is_pipeline_last_stage(ignore_virtual=True)
                                and k % (num_model_chunks * pipeline_parallel_size)
                                >= (num_model_chunks - 1) * pipeline_parallel_size
                            )
                        ):
                            forward_sent_indices.append(k)
                ...
                if has_tensor_to_send:
                    break
            if has_tensor_to_send:
                shm_tensor.forward_create_shared_memory(forward_finished_tensors[k], torch.distributed.get_global_rank(group=mpu.get_pipeline_model_parallel_group(), group_rank=(pipeline_parallel_rank + 1) % pipeline_parallel_size))
                forward_sent_count += 1
                print(f"rank {torch.distributed.get_rank()} forward send: {forward_sent_count}/{total_num_microbatches_to_send_forward}")
                if forward_sent_count == total_num_microbatches_to_send_forward:
                    return

    def forward_recv(forward_ready_tensors):
        forward_recved_indices = []
        # torch.cuda.set_device(torch.distributed.get_rank())
        ...
        while True:
            for i in range(num_model_chunks): # chunk number in one GPU
                for j in range(i * pipeline_parallel_size, total_num_microbatches, pipeline_parallel_size * num_model_chunks):
                    for k in range(j, j + pipeline_parallel_size):
                        if (
                            k not in forward_recved_indices
                            and not (
                            ...
                            tensor = shm_tensor.forward_read_shared_memory(np.prod(tensor_shape), torch.distributed.get_global_rank(group=mpu.get_pipeline_model_parallel_group(), group_rank=pipeline_parallel_rank))
                            forward_recved_count += 1
                            print(f"rank {torch.distributed.get_rank()} forward recv: {forward_recved_count}/{total_num_microbatches_to_recv_forward}")
                            forward_recved_indices.append(k)
                            forward_ready_tensors[k] = tensor.reshape(tensor_shape)
                            if forward_recved_count == total_num_microbatches_to_recv_forward:
                                return

    def backward_send(backward_finished_tensors):
        ...
        while True:
            ...
            for i in range(num_model_chunks): # chunk number in one GPU
                for j in range(i * pipeline_parallel_size, total_num_microbatches, pipeline_parallel_size * num_model_chunks):
                    for k in range(j, j + pipeline_parallel_size):
                        if (
                            backward_finished_tensors[k] is not None
                            and k not in backward_sent_indices
                            and not (
                                parallel_state.is_pipeline_first_stage(ignore_virtual=True)
                                and k % (num_model_chunks * pipeline_parallel_size)
                                >= (num_model_chunks - 1) * pipeline_parallel_size
                            ...
                            backward_sent_indices.append(k)
                            has_tensor_to_send = True
                            break
                    if has_tensor_to_send:
                        break
                if has_tensor_to_send:
                    break
            if has_tensor_to_send:
                shm_tensor.backward_create_shared_memory(backward_finished_tensors[k], torch.distributed.get_global_rank(group=mpu.get_pipeline_model_parallel_group(), group_rank=(pipeline_parallel_rank - 1) % pipeline_parallel_size))
                backward_sent_count += 1
                print(f"rank {torch.distributed.get_rank()} backward send: {backward_sent_count}/{total_num_microbatches_to_send_backward}")
                if backward_sent_count == total_num_microbatches_to_send_backward:
                    return

    def backward_recv(backward_ready_tensors):
        backward_recved_indices = []
        # torch.cuda.set_device(torch.distributed.get_rank())
        ...
        while True:
            for i in range(num_model_chunks): # chunk number in one GPU
                for j in range(i * pipeline_parallel_size, total_num_microbatches, pipeline_parallel_size * num_model_chunks):
                    for k in range(j, j + pipeline_parallel_size):
                        if (
                            k not in backward_recved_indices
                            and not (
                                parallel_state.is_pipeline_last_stage(ignore_virtual=True)
                                and k % (num_model_chunks * pipeline_parallel_size)
                                < pipeline_parallel_size
                            ...
                            tensor = shm_tensor.backward_read_shared_memory(np.prod(tensor_shape), torch.distributed.get_global_rank(group=mpu.get_pipeline_model_parallel_group(), group_rank=pipeline_parallel_rank))
                            backward_recved_count += 1
                            print(f"rank {torch.distributed.get_rank()} backward recv: {backward_recved_count}/{total_num_microbatches_to_recv_backward}")
                            backward_recved_indices.append(k)
                            backward_ready_tensors[k] = tensor.reshape(tensor_shape)
    ...
    # start the threads
    shm_tensor_new_rdma.thread_pool(num_model_chunks, pipeline_parallel_size, total_num_microbatches, parallel_state.is_pipeline_last_stage(ignore_virtual=True), parallel_state.is_pipeline_first_stage(ignore_virtual=True), torch.distributed.get_global_rank(group=mpu.get_pipeline_model_parallel_group(), group_rank=(pipeline_parallel_rank + 1) % pipeline_parallel_size), torch.distributed.get_global_rank(group=mpu.get_pipeline_model_parallel_group(), group_rank=(pipeline_parallel_rank - 1) % pipeline_parallel_size), torch.distributed.get_global_rank(group=mpu.get_pipeline_model_parallel_group(), group_rank=pipeline_parallel_rank), total_num_microbatches_to_send_forward, total_num_microbatches_to_recv_forward, total_num_microbatches_to_send_backward, total_num_microbatches_to_recv_backward, np.prod(tensor_shape), forward_only)
    # send_tensor_test(5)
    """forward_send_thread = multiprocessing.Process(target=forward_send, args=(forward_finished_tensors,))
    forward_recv_thread = multiprocessing.Process(target=forward_recv, args=(forward_ready_tensors,))
    backward_send_thread = multiprocessing.Process(target=backward_send, args=(backward_finished_tensors,))
    backward_recv_thread = multiprocessing.Process(target=backward_recv, args=(backward_ready_tensors,))
    forward_send_thread.start()
    forward_recv_thread.start()
    if not forward_only:
        backward_send_thread.start()
        backward_recv_thread.start()"""
    ...
    while computed_forward_count < total_num_microbatches or (
            ...
            for i, k in enumerate(index_list):
                ...
                for j in range(i * pipeline_parallel_size, total_num_microbatches, pipeline_parallel_size * num_model_chunks):
                    for k in range(j, j + pipeline_parallel_size):
                        ...
                        if k not in computed_backward_indices and forward_finished_tensors[dual_index] is not None and (forward_ready_tensors[dual_index] is not None or (parallel_state.is_pipeline_first_stage(ignore_virtual=True) and dual_index % (num_model_chunks * pipeline_parallel_size) < pipeline_parallel_size)) and (
                            backward_ready_tensors[k] is not None
                            or (
                                parallel_state.is_pipeline_last_stage(ignore_virtual=True)
                                and k % (num_model_chunks * pipeline_parallel_size) < pipeline_parallel_size
                            )
                        ):
                            index_to_compute = k
        ...
        for k in index_list:
            ...
            for j in range(i * pipeline_parallel_size, total_num_microbatches, pipeline_parallel_size * num_model_chunks):
                for k in range(j, j + pipeline_parallel_size):
                    if k not in computed_forward_indices and (
                        forward_ready_tensors[k] is not None
                        or (
                            parallel_state.is_pipeline_first_stage(ignore_virtual=True)
                            and k % (num_model_chunks * pipeline_parallel_size) < pipeline_parallel_size
                        )
        ...
        if can_forward_compute:
            ...
            forward_finished_tensors[index_to_compute] = forward_step(
                ...
                num_microbatches,
                forward_ready_tensors[index_to_compute],
                forward_data_store,
                config,
                collect_non_loss_data,
                checkpoint_activations_microbatch,
    ...
    for k in range(num_warmup_microbatches):
        ...
        print(f"coompute time: {end - start}")

...
def _normalize_gpt2_keys_to_transformer(sd: dict) -> dict:
    """
    将没有 'transformer.' 前缀的 GPT-2 权重键名标准化为带前缀的形式。
    支持键示例：wte.weight / wpe.weight / h.0.attn.c_attn.weight / ln_f.weight
    转成：transformer.wte.weight / transformer.wpe.weight / transformer.h.0.attn.c_attn.weight / transformer.ln_f.weight
    """
    # 如果本来就有标准前缀，直接返回
    if any(k.startswith("transformer.") for k in sd.keys()):
        return sd

    new_sd = {}
    for k, v in sd.items():
        # lm_head.* 本来就不在 transformer 下，保持原样
        if k.startswith("lm_head."):
            new_sd[k] = v
            continue

        # 常见几种前缀：wte., wpe., h., ln_f.
        if k.startswith(("wte.", "wpe.", "h.", "ln_f.")):
            new_sd["transformer." + k] = v
        else:
            # 其它少见键（基本不会出现），也补上 transformer.，避免漏网
            new_sd["transformer." + k] = v
    return new_sd
...
def convert_checkpoint_from_transformers_to_megatron(args):
    ...
    state_dict = _normalize_gpt2_keys_to_transformer(state_dict)
    ...
    # print("Initialized output state dict---------------")
    ...
    for pp_rank in range(args.target_pipeline_model_parallel_size):
        ...
        for tp_rank in range(args.target_tensor_model_parallel_size):
            ...
            output_state_dict[tp_rank]["iteration"] = 1
            checkpoint_dir = (
